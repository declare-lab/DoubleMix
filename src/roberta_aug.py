...
from transformers.models.roberta.modeling_roberta import RobertaPreTrainedModel, RobertaLayer, RobertaEmbeddings, RobertaPooler

class RobertaAug4Mix(RobertaPreTrainedModel):
    def __init__(self, config):
        super(RobertaAug4Mix, self).__init__(config)
...
class Roberta_aug(nn.Module):
    def __init__(self, num_labels=3, output_attentions=False):
        super(Roberta_aug, self).__init__()
        self.output_attentions = output_attentions
        self.model = RobertaAug4Mix.from_pretrained('roberta-base', output_attentions=self.output_attentions)          

        self.linear = nn.Sequential(nn.Linear(768, 128),
                                    nn.Tanh(),
                                    nn.Linear(128, num_labels))

    def forward(self, inputs, aug_inputs1=None, aug_inputs2=None, lam=None, ws=None, mix_layer=None):   
        if self.output_attentions:
            seq_output, pooled_output, attentions = self.model(inputs, aug_inputs1, aug_inputs2, lam, ws, mix_layer)        
        else:          
            seq_output, pooled_output = self.model(inputs, aug_inputs1, aug_inputs2, lam, ws, mix_layer)
            attentions = None
